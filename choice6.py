...
def main():
    # 1) Load calibration intrinsics and distortion
    data = np.load("calibration_results.npz", allow_pickle=True)
    K = data["cameraMatrix_run1"]
    d = data["distCoeffs_run1"]

    # 2) Prepare the chessboard 3D points (world frame = checkerboard frame)
    objp = create_object_points()

    # 3) Collect camera centers for each image
    cam_centers = []
    labels = []
    ...
    for fname in images:
        ...
        # 5) Refine corners for more stable pose
        corners = cv2.cornerSubPix(
            gray, corners, (11, 11), (-1, -1),
            (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
        )

        # 6) Estimate pose (rvec, tvec) that maps world points -> camera points
        ok, rvec, tvec = cv2.solvePnP(objp, corners, K, d)
        if not ok:
            print(f"{fname}: solvePnP failed (skipping)")
            continue

        # 7) Convert rotation vector to rotation matrix
        R, _ = cv2.Rodrigues(rvec)

        # 8) Camera center in world coordinates:
        #    X_cam = R * X_world + t  =>  camera center C_world = -R^T * t
        C = -R.T @ tvec
    ...
    # 9) Print a quick reflection / interpretation
